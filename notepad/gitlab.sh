...
Install_gitlab()
{
	...
	if [ "$isEl7" != '' ];then
		...
		if [ ! -f gitlab-ce-11.1.4-ce.0.el7.x86_64.rpm ];then
			wget -O gitlab-ce-11.1.4-ce.0.el7.x86_64.rpm $download_Url/src/gitlab-ce-11.1.4-ce.0.el7.x86_64.rpm -T 5
		else
			wget -c $download_Url/src/gitlab-ce-11.1.4-ce.0.el7.x86_64.rpm -T 5
		fi
		...
		rm -f gitlab-ce-11.1.4-ce.0.el7.x86_64.rpm
	...
	if [ ! -f '/www/server/panel/class/panelPlugin.py' ];then
		wget -O /www/server/panel/class/panelPlugin.py $download_Url/panel/class/panelPlugin.py -T 20
...
exec chpst -P /opt/gitlab/embedded/sbin/gitlab-web -p /var/opt/gitlab/nginx" > /opt/gitlab/service/nginx/run
	...
	mkdir -p $pluginPath
	wget -O $pluginPath/gitlab_main.py $download_Url/install/lib/plugin/gitlab/gitlab_main.py
	wget -O $pluginPath/icon.png $download_Url/install/lib/plugin/gitlab/icon.png
	wget -O $pluginPath/index.html $download_Url/install/lib/plugin/gitlab/index.html
	wget -O $pluginPath/info.json $download_Url/install/lib/plugin/gitlab/info.json
	wget -O $pluginPath/install.sh $download_Url/install/lib/plugin/gitlab/install.sh
